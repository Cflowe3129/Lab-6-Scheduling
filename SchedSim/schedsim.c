...
{ 
   ...
   */
   ...
   int x;
   for(x = 0; x < n; x++) {  //copy plist[] -> rem_bt && initialize plist[] to 0
     rem_bt[x] = plist[x].bt;
     plist[x].wt = 0;
//      printf("init\n\n");
   }
   ...
   int time = 0;
   bool done = false;
   while(!done) { //Keep traversing the all processes while all processes are not done
//      printf("inside loop\n\n");
     done = true;
     for(x = 0; x < n; x++) {
...
{ 
     ...
     */
     ...
     int rem_bt[n];
     int x;
     for(x = 1; x < n ;x++) { //copy plist[] -> rem_bt
       rem_bt[x] = plist[x].bt;
     }
     ...
     while(y < n) {
       int minimum_index = 0;
       for(x = 1; x < n; x++) { //find process with minimum remaining time 
         if(rem_bt[x] < rem_bt[minimum_index]) {
           minimum_index = 1;
         }
       }
       ...
       if(rem_bt[minimum_index] == 0) {
         y += 1;
         minimum_time = time + 1;
         plist[minimum_index].wt = minimum_time - plist[minimum_index].bt;
         rem_bt[minimum_index] = 1000;
       }
       time+= 1;
     }
